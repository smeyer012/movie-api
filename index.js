...
let allowedOrigins = ['http://localhost:8080', 'http://localhost:1234', 'https://myflix-smeyer.netlify.app'];
app.use(cors({
...
app.get('/:dataType/:data', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  if (ObjectId.isValid(req.params.data)) {
    ...
    myModels[req.params.dataType].findOne({
      $or: [
        { Name: dataName },
        { Username: dataName },
        { Title: dataName }
...
app.put('/movies/:ID', passport.authenticate('jwt', { session: false }), (req, res) => {
  myModels['movies'].findOneAndUpdate({ _id: req.params.ID }, {
    $set:
    {
